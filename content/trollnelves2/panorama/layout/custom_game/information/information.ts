...
{
    const GuidesPanels = $("#GuidesPanels");
	for (tbl_info of information_panels) 
    {
        const info_window = $.CreatePanel("Panel", GuidesPanels, "");
        info_window.AddClass("info_window");

        const info_button = $.CreatePanel("Panel", info_window, "");
        info_button.AddClass("info_button");

        const info_button_label = $.CreatePanel("Label", info_button, "");
        info_button_label.AddClass("info_button_label");
        info_button_label.text = $.Localize("#" + tbl_info[0]);

        const info_button_arrow = $.CreatePanel("Panel", info_button, "");
        ...
        const info_video_and_description = $.CreatePanel("Panel", info_window, "");
        info_video_and_description.AddClass("info_video_and_description");

        const info_description = $.CreatePanel("Label", info_video_and_description, "");
