...
function UpdateListSelected() {
    ...
    const active_table = (players_activated_spells === null || players_activated_spells === void 0 ? void 0 : players_activated_spells[player_id]) || {};
    ...
    for (const panel of $("#AllPerksList").Children()) {
        ...
        if (label) {
            const index = (_a = Object.entries(active_table).find(([_, name]) => name === spell_name)) === null || _a === void 0 ? void 0 : _a[0];
            label.text = index || "";
        }
...
function VoteHud_PlayerHasSpell(spell) {
    if (player_table) {
        for (var entry_id in player_table) {
...
function GetMaxAllowedPerksForLocalPlayer() {
    ...
    if (team === 3)
        return 3;
    const active = players_activated_spells === null || players_activated_spells === void 0 ? void 0 : players_activated_spells[id];
    if (!active)
        return 1;
    for (let i = 1; i <= 3; i++) {
...
function UpdateModVotes(data) {
    ...
    let yesPercent = 0;
    for (var vote_id in data) {
        let info = data[vote_id];
        if (info.map_id == 1) {
