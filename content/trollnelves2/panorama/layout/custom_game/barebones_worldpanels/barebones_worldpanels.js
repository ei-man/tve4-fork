    ...
    for (var k in changes) {
        ...
        for (var j in changes[k]) {
            if (j == "position") {
                ...
                if (wp.panel.onDataUpdate !== null) {
                    wp.panel.onDataUpdate(wp.panel.Data);
                }
                wp[j] = changes[k][j];
            }
            else
...
function PositionPanels() {
    ...
    for (var k in panels) {
        ...
        // //var scale = (1024 + ((Game.GetScreenWidth() * Game.GetScreenHeight() / 10000) - 78)) / sh;
        // var scaleH = 1080 / sh;
        // var scaleW = 1700 / sw;
        //
        // var x = scaleW * wx + wp.offsetX;
        // wx = wx + wp.offsetX
        // var y = scaleH * wy + wp.offsetY;
        // wy = wy + wp.offsetY
        //
