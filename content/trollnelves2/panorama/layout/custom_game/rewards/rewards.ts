...
{
	const Reward = $.CreatePanel("Panel", $("#RewardsRow"), "reward_day_" + day);
	Reward.AddClass("Reward");

    const RewardLight = $.CreatePanel("Panel", Reward, "");
	RewardLight.AddClass("RewardLight");

	const DayNumber = $.CreatePanel("Label", Reward, "");
	DayNumber.AddClass("DayNumber");
	DayNumber.text = day + " " + $.Localize("#rewards_days");

	const RewardIcon = $.CreatePanel("Panel", Reward, "");
	RewardIcon.AddClass("RewardIcon");

	const RewardIconImage = $.CreatePanel("Panel", RewardIcon, "");
    ...
    const RewardLine = $.CreatePanel("Panel", $("#RewardsRow"), "");
	RewardLine.AddClass("RewardLine");

	const RewardInfoLabel = $.CreatePanel("Label", Reward, "");
	RewardInfoLabel.AddClass("RewardInfoLabel");
    
	const reward_info = reward_table[2];
    ...
    const RewardClaimButton = $.CreatePanel("Panel", Reward, "RewardClaimButton");
	RewardClaimButton.AddClass("RewardClaimButton");

    const RewardClaimButtonLabel = $.CreatePanel("Label", RewardClaimButton, "RewardClaimButtonLabel");
...
function RecieveReward(claim_panel, reward_panel, type_reward, reward_count)
{
    ...
    const RewardClaimButtonLabel = claim_panel.GetChild(0);
    RewardClaimButtonLabel.text = $.Localize("#reward_recieved");
