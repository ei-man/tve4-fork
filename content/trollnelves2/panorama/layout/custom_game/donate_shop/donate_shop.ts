...
function InitMainPanel() 
{
	...
	for (let i = 0; i <= Object.keys(Items_recomended).length; i++) 
    {
...
{
    const is_chest = IsItemChest(table[i][1]);
	...
	const Recom_item = $.CreatePanel("Panel", panel, "");
	Recom_item.AddClass("RecomItem");

    ItemTooltipShow(Recom_item, table[i][5]);

    const ItemImage = $.CreatePanel("Panel", Recom_item, "");
    ...
    if (isLabelItem)
    {
        ...
        const labelItemText = $.CreatePanel("Label", ItemImage, "");
    ...
    else
    {
        ...
        const BuyItemPanel = $.CreatePanel("Panel", Recom_item, "BuyItemPanel");
        BuyItemPanel.AddClass("BuyItemPanel");

    if (is_inventory && is_chest)
    {
        const chest_count_label = $.CreatePanel("Label", Recom_item, "");
	...
	const ItemPrice = $.CreatePanel("Panel", BuyItemPanel, "ItemPrice");
    ...
    if (!is_inventory)
    {
        if (table[i][3] == "9999999")
        {
            ...
            const PriceLabel = $.CreatePanel("Label", ItemPrice, "PriceLabel");
        ...
        {
            const PriceIcon = $.CreatePanel("Panel", ItemPrice, "PriceIcon");
    ...
    else
    {
        ...
        else
        {
            ...
            const is_item_activated = IsItemActivated(check_item_id);
            const PriceLabel = $.CreatePanel("Label", ItemPrice, "PriceLabel");
            PriceLabel.AddClass("PriceLabel");
...
{
    for ( const item in player_table[1] )
    {
       	if (item == id) 
        {
       		return true;
...
{
    for ( const chest in player_table[4] )
    {
...
function IsItemActivated(id)
{
    ...
    {
        const item_id = player_active_items[slot];
        if (item_id == id)
        {
            return true;
        }
...
function BuyCurrencyPanelActive()
{
	...
	const Panel_for_desc = $.CreatePanel("Label", $("#ItemInfoBody"), "Panel_for_desc");
	Panel_for_desc.AddClass("Panel_for_desc");

	const Item_desc = $.CreatePanel("Label", Panel_for_desc, "Item_desc");
	Item_desc.AddClass("Item_desc");
	Item_desc.text = $.Localize( "#buy_currency_description" );

	const columns = $.CreatePanel("Panel", $("#ItemInfoBody"), "columns");
	columns.AddClass("columns_donate");

	const column_1 = $.CreatePanel("Panel", columns, "column_1");
	column_1.AddClass("column_donate");

	const column_2 = $.CreatePanel("Panel", columns, "column_2");
...
{
    const ChestCounterPanel = $("#ChestCounterPanel");
    const ChestCounter = $("#ChestCounter");
    let chest_counter = 0;
    for (let i = 0; i <= Object.keys(chests_table).length; i++) 
    {
...
function SetItemBuyFunction(panel, table)
{
    panel.SetPanelEvent("onactivate", function()
    {
		...
		const str = table[4].replace(/[^a-zа-яё]/gi, "");
		Item_desc.text = $.Localize("#" + str + "_description" );

		if (table[4].indexOf("chest") == 0) 
        {
            $("#info_item_buy").SetHasClass("IsChest", true);
            const chest_table = GetChestInfo(Number(table[1]));
			const ChestItemPreview = $.CreatePanel("Panel", $("#ItemInfoBody"), "ChestItemPreview");
			ChestItemPreview.AddClass("ChestItemPreview");
			for (let i = 0; i <= Object.keys(Items_ALL).length; i++) 
            {
                if (Items_ALL[i])
                {
		...
		const BuyItemPanel = $.CreatePanel("Panel", $("#ItemInfoBody"), "BuyItemPanel");
		BuyItemPanel.AddClass("BuyItemPanelInfo");

		const PriceLabel = $.CreatePanel("Label", BuyItemPanel, "PriceLabel");
...
{
	const Chest_in_item = $.CreatePanel("Panel", panel, "item_" + currency);
	...
	const ItemImage = $.CreatePanel("Panel", Chest_in_item, "");
...
{
	for (const item_chest_num in table_chest[1] ) 
    {
        const item_chest_info = table_chest[1][item_chest_num];
		if (table[i][1] == item_chest_info[1]) 
        {
			if (!panel.FindChildTraverse("item_" + item_chest_info[1])) 
            {
				const Chest_in_item = $.CreatePanel("Panel", panel, "item_" + item_chest_info[1]);
				Chest_in_item.AddClass("Chest_in_item_preview");
                ...
                const color_rare = CHEST_GRADIENT_COLORS[table[i][7]];
                if (color_rare)
                {
...
function SetOpenChestPanel(panel, table)
{
    panel.SetPanelEvent("onactivate", function() 
    {
        ...
        const chest_information = $.CreatePanel("Panel", $("#ChestBodyInfo"), "");
		chest_information.AddClass("chest_information");

		const ChestImage = $.CreatePanel("Panel", chest_information, "");
        ...
        const ChestName = $.CreatePanel("Label", chest_information, "");
		ChestName.AddClass("ChestNameInfoLabel");
        ChestName.text = $.Localize("#" +  table[4] );

        const chest_dropped_panel = $.CreatePanel("Panel",  $("#ChestBodyInfo"), "");
		chest_dropped_panel.AddClass("chest_dropped_panel");

        const chest_dropped_panel_line = $.CreatePanel("Panel", chest_dropped_panel, "chest_dropped_panel_line");
		chest_dropped_panel_line.AddClass("chest_dropped_panel_line");

        const chest_dropped_panel_arrow = $.CreatePanel("Panel", chest_dropped_panel, "");
		chest_dropped_panel_arrow.AddClass("chest_dropped_panel_arrow");

		const ChestAllRewardsPanel = $.CreatePanel("Panel",  $("#ChestBodyInfo"), "ChestAllRewardsPanel");
		ChestAllRewardsPanel.AddClass("ChestAllRewardsPanel");

		const OpenChestButton = $.CreatePanel("Panel",  $("#ChestBodyInfo"), "OpenChestButton");
		OpenChestButton.AddClass("OpenChestButton");

		const OpenChest_Label = $.CreatePanel("Label", OpenChestButton, "");
...
{
    const items_chest = chest_table[1];

    for (let i = 0; i <= 50; i++)
    {
        const randomIndex = Math.floor(Math.random() * Object.keys(items_chest).length) + 1;
        const randomElement = items_chest[randomIndex];
...
function CreateItemInRoll(main_panel, item_info, delay_count, roll, drop_slot, c)
{
    $.Schedule( 0.01 * delay_count, function()
    {
        ...
        const chest_roll_item = $.CreatePanel("Panel", main_panel, panel_id);
        chest_roll_item.AddClass("chest_roll_item");

        const chest_roll_img = $.CreatePanel("Panel", chest_roll_item, "");
        chest_roll_img.AddClass("chest_roll_img");

        const ItemImage = $.CreatePanel("Panel", chest_roll_img, "ItemImage");
        ItemImage.AddClass("chest_roll_item_Image");

        const ItemRollRarity = $.CreatePanel("Panel", chest_roll_item, "");
        ItemRollRarity.AddClass("ItemRollRarity");

        const item_info_real = GetItemInfo(item_info[1]);
        ...
        $.Schedule( 0.1, function()
        {
            if (drop_slot)
            {
                const check_pos = chest_roll_item.style.position;
                const SpaceFind = check_pos.indexOf("px");
                const center_panel = Number(check_pos.substring(0, SpaceFind));
...
{
    const chest_dropped_panel_line = $("#ChestBodyInfo").FindChildTraverse("chest_dropped_panel_line");
...
{
    const OpenChestButton = $("#ChestBodyInfo").FindChildTraverse("OpenChestButton");
    if (OpenChestButton)
    {
...
function RewardRequest(data) 
{
    ...
    const current = 0;
    const randomly_max_distance = Math.floor(DROP_POS[0]);
    const chest_dropped_panel_line = $("#ChestBodyInfo").FindChildTraverse("chest_dropped_panel_line");
    if (chest_dropped_panel_line)
    {
        const ItemImage = chest_dropped_panel_line.FindChildTraverse("dropped_item").FindChildTraverse("ItemImage");
        const item_info_real = GetItemInfo(reward);
        if (item_info_real != null)
        {
            if (BOX_SHADOW_COLORS[item_info_real[7]])
            {
...
function SetRewardView(drop_id)
{
	...
	const RewardIcon = $.CreatePanel("Panel",  $("#chest_opened_animation"), "RewardIcon");
	RewardIcon.AddClass("ChestOpenImageItem");
	let icon;
	if (drop_id) 
    {
        const item_info_real = GetItemInfo(drop_id);
        if (item_info_real != null)
        {
	...
	const AcceptButton = $.CreatePanel("Panel",  $("#chest_opened_animation"), "AcceptButton");
	AcceptButton.AddClass("AcceptButton");
	AcceptButton.SetPanelEvent("onactivate", function() { CloseChest(); CloseOpenChest(); } );
	
	const LabelAccept = $.CreatePanel("Label", AcceptButton, "LabelAccept");
