...
function UpdateQuestAfter() {
    if (HIDE_QUESTS_PANEL) {
        ...
        if (qp)
            qp.style.visibility = "collapse";
        const swap = $("#QuestPanelSwap");
        if (swap)
            swap.style.visibility = "collapse";
        const shadow = $("#PanelShadow") || root.FindChildrenWithClassTraverse("PanelShadow")[0];
        if (shadow)
            shadow.style.visibility = "collapse";
        return;
    }
    if (UPDATED_QUEST_DAY[Players.GetLocalPlayer()] != null) {
        return;
    }
...
if (!HIDE_QUESTS_PANEL) {
    ...
    if (panelShadow)
        panelShadow.style.visibility = "collapse";
}
